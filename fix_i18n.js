...
// Parse all keys from i18n.js into a lookup: stripped_en_text -> {key, pt}
const translationMap = new Map();
const keyEntries = [];

// Extract all key: { en: '...', pt: '...' } entries
...
while ((match = entryRegex.exec(i18nContent)) !== null) {
  ...
  keyEntries.push({ key, en, pt });
  // Store by stripped text
  const stripped = en.replace(/<[^>]*>/g, '').trim();
  if (!translationMap.has(stripped)) {
    translationMap.set(stripped, []);
  }
  translationMap.get(stripped).push({ key, pt });
}

// Also build a map from key -> {en, pt}
const keyMap = new Map();
for (const entry of keyEntries) {
  keyMap.set(entry.key, { en: entry.en, pt: entry.pt });
}

// Find the closing } of translations object
const insertPos = i18nContent.lastIndexOf('};');
const beforeInsert = i18nContent.substring(0, insertPos);
const afterInsert = i18nContent.substring(insertPos);

// Track new keys to add
let newKeys = [];
let totalRestored = 0;
let totalNew = 0;

// Process each case 01-07
const cases = [
...
function translateToPT(text) {
  const translations = {
    ...
    "18% losses": "18% de perdas",
    "Core outcome": "Resultado principal",
    "90-day recovery": "Recuperação em 90 dias",
  ...
  console.error(`  [WARN] No PT-BR translation for: "${text.substring(0, 80)}..."`);
  return text; // fallback
}

// Split into section context
function extractTextContent(html) {
  return html.replace(/<[^>]*>/g, '').trim();
}

// Find matching key in translationMap
function findMatchingKey(text) {
  if (translationMap.has(text)) {
    const matches = translationMap.get(text);
...
for (const c of cases) {
  const htmlPath = path.join('cases', c.dir, 'index.html');
  ...
  while ((sectionMatch = sectionRegex.exec(html)) !== null) {
    ...
    // Find all elements without data-i18n: h2, h3, p, li, td, th
    const elemRegex = /<(h2|h3|p|li|td|th)(\s[^>]*)?>([\s\S]*?)<\/\1>/g;
    let elemMatch;
    
    while ((elemMatch = elemRegex.exec(sectionContent)) !== null) {
      const fullTag = elemMatch[0];
      const tagName = elemMatch[1];
      const attrs = elemMatch[2] || '';
      const innerContent = elemMatch[3];
      
      // Skip if it already has data-i18n
      if (attrs.includes('data-i18n=')) continue;
      
      // Extract text content
      const textContent = extractTextContent(innerContent);
      if (!textContent) continue;
      
      // For h2 section titles, use sec{num}-title
      let keyName = null;
      let ptTranslation = null;
      
      if (tagName === 'h2') {
        keyName = `sec${sectionNum}-title`;
        if (keyMap.has(keyName)) {
          // Already exists in i18n.js
        } else {
          const titles = {
          ...
          ptTranslation = titles[textContent] || textContent;
        }
      } else {
        // Look for matching text in i18n.js
        const match = findMatchingKey(textContent);
        if (match) {
          keyName = match.key;
        } else {
          ...
          let type = tagName;
          if (tagName === 'h3') type = 'h3';
          
          // Count existing keys for this case-section-type combination
          const prefix = `c${caseNum}-s${sectionNum}-${type}`;
          let count = 1;
          ...
          const pt = translateToPT(textContent);
          newCaseEntries.push({ key: keyName, en: textContent, pt });
          caseNew++;
        }
      }
      
      if (keyName) {
        // Build new tag with data-i18n
        const newTag = `<${tagName} data-i18n="${keyName}"${attrs ? ' ' + attrs.trim() : ''}>${innerContent}</${tagName}>`;
...
if (newKeys.length > 0) {
  ...
  // Insert before the closing }; of translations
  ...
  console.log(`\nAdded ${newKeys.length} new keys to i18n.js`);
}

console.log(`\nSummary: ${totalRestored} data-i18n attributes restored, ${totalNew} new keys created`);
